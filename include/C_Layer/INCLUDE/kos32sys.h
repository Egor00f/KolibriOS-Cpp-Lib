...
#define SHM_WRITE 0x01
	...
	static inline void end_draw(void)
	{
		__asm__ __volatile__(
			"int $0x40" ::"a"(12), "b"(2));
	};

	static inline void sys_create_window(int x, int y, int w, int h, const char *name,
	...
	static inline void define_button(uint32_t x_w, uint32_t y_h, uint32_t id, uint32_t color)
	{
		__asm__ __volatile__(
			"int $0x40" ::"a"(8),
			"b"(x_w),
	...
	static inline void draw_line(int xs, int ys, int xe, int ye, color_t color)
	{
		__asm__ __volatile__(
	...
	static inline void draw_bar(int x, int y, int w, int h, color_t color)
	{
		__asm__ __volatile__(
	...
	static inline void draw_bitmap(void *bitmap, int x, int y, int w, int h)
	{
		__asm__ __volatile__(
	...
	static inline void draw_text_sys(const char *text, int x, int y, int len, color_t color)
	{
		__asm__ __volatile__(
			"int $0x40" ::"a"(4), "d"(text),
			"b"((x << 16) | y),
	...
	static inline uint32_t get_skin_height(void)
	{
		uint32_t height;
	...
	static inline pos_t get_mouse_pos(int origin)
	{
		pos_t val;
	...
	static inline uint32_t get_mouse_buttons(void)
	{
		uint32_t val;
	...
	static inline uint32_t get_mouse_wheels(void)
	{
		uint32_t val;
	...
	static inline uint32_t wait_for_event(uint32_t time)
	{
		uint32_t val;
		__asm__ __volatile__(
			"int $0x40"
	...
	static inline uint64_t get_ns_count(void)
	{
		uint64_t val;
		__asm__ __volatile__(
			"int $0x40"
	...
	static inline int *user_unmap(void *base, size_t offset, size_t size)
	{
		int *val;
		__asm__ __volatile__(
			"int $0x40"
...
#define X_Y(x, y) (((x) << 16) | (y))
	...
	static inline int kol_clip_num()
	{
	...
	static inline char *kol_clip_get(int n)
	...
	static inline int kol_clip_set(int n, char buffer[])
	{
		register uint32_t val;
	...
	static inline int kol_clip_pop()
	{
		register uint32_t val;
	...
	static inline int kol_clip_unlock()
	{
		register uint32_t val;
	...
	static inline uint32_t get_mouse_eventstate(void)
	{
		uint32_t val;
	...
	static inline uint32_t set_event_mask(uint32_t mask)
	{
		register uint32_t val;
	...
	static inline int start_thread(thread_proc proc, char *stack_top)
	{
		register int val;
	...
	static inline void ipc_set_area(void *buf, int bufsize)
	{
		asm volatile("int $0x40" ::"a"(60), "b"(1), "c"(buf), "d"(bufsize));
	}

	static inline int ipc_send_message(int pid_reciever, void *data, int datalen)
	{
		register int val;
	...
	static inline void *shm_open(char *shm_name, int msize, int flags, int *retsz)
	{
		register int val, cod;
		asm volatile("int $0x40" : "=a"(val), "=d"(cod) : "a"(68), "b"(22), "c"(shm_name), "d"(msize), "S"(flags));

		if (retsz)
			*retsz = cod; // errcode if NULL or memsize when open
		return (void *)val;
	}

	static inline void shm_close(char *shm_name)
	{
		asm volatile("int $0x40" ::"a"(68), "b"(23), "c"(shm_name));
	}

	static inline int start_app(char *app_name, char *args)
	{
		file_op_t file_op;
	...
	static inline int font_size(int color)
