...
{
    _ksys_draw_line(coord.x, coord.y, b.x, b.y, color);
}

void graphic::DrawLine(const UI::Coord &coord, unsigned lenght, unsigned short angle, const Color::Color &color)
{
    _ksys_draw_line(coord.x, coord.y, coord.x + (lenght * cos(angle)), coord.y + (lenght * sin(angle)), color);
}
...
void graphic::DrawCircle(const UI::Coord &coord, const unsigned &Radius, const unsigned &detalization, const Color::Color &color)
{
    UI::Coord buff;
    unsigned b = Radius;
    unsigned c = 0;
...
void graphic::DrawCircleFill(const UI::Coord &coord, const unsigned &Radius, const unsigned &detalization = 36, const Color::Color &color)
{
